...
import { getConverter } from "../converters/converterRegistry.js";
...
export default class FigmaToCodeService {
  ...
  async parseNode(node, fileId) {
    ...
    if (this.componentName !== this.existingComponentName) {
      if (
        ...
        this.imports.add(this.existingComponentName);
  ...
  codeConverter(framework) {
    const ConverterClass = getConverter(framework);

    if (!ConverterClass) {
      throw new Error(`No converter registered for framework: ${framework}`);
    }
